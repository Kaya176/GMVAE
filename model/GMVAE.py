...
class GMVAE:
  ...
  def unlabeled_loss(self, data, out_net, label):
    ...
    # categorical loss
    loss_cat = -self.losses.entropy(logits, prob_cat) - np.log(1/3)#- np.log(0.1)
    # loss_cat = -self.losses.entropy(logits, label) - np.log(0.1)
    # loss_cat=nn.BCELoss()(prob_cat,label)- np.log(0.1)

    #Clf loss?

    # loss_clf= nn.BCELoss()(prob_cat,label)
  ...
  def train_epoch(self, optimizer, data_loader):
    ...
    for (data, labels, labels_oh) in data_loader:
      if self.cuda == 1:
        data = data.cuda()
        oh_labels=labels_oh.cuda()
      ...
      unlab_loss_dic = self.unlabeled_loss(data, out_net, oh_labels) 
  ...
  def train(self, train_loader, val_loader,test_loader):
    ...
    for epoch in range(1, self.num_epochs + 1):
      ...
      wandb.log({'train loss':train_loss,'train_rec':train_rec,'train_gauss':train_gauss,'train_cat':train_cat})
      wandb.log({'val loss':val_loss,'val_rec':val_rec,'val_gauss':val_gauss,'val_cat':val_cat})
      wandb.log({'train_acc':train_acc,'val_acc':val_acc,'train_nmi':train_nmi,'val_nmi':val_nmi})
      ...
      #Print Test
      test_loss, test_rec, test_gauss, test_cat, test_acc, test_nmi = self.test(test_loader, True)
      wandb.log({'test acc':test_acc})
      print("Test {:.3f}".format(test_acc))
  ...
  def pred_label(self, data_loader, return_loss=False):
    """Test the model with new data
    ...
    # compute metrics
    # accuracy = 100.0 * self.metrics.cluster_acc(predicted_labels, true_labels)
    acc,row,col = self.metrics.cluster_acc(predicted_labels, true_labels,return_label=True)
    #row : pred, col: actual
    #row, col combination
    # assignment: [(row[i],c[i]) for i in range(len(row))]
    assignments= [(row[i],col[i]) for i in range(len(row))]
    y_pred=np.zeros_like(predicted_labels)
    for a in assignments:
      y_pred[predicted_labels==a[0]]=a[1]
    
    # print(row)
    # print(col)
    # print([(row[i],col[i]) for i in range(len(row))])
    print(acc)
